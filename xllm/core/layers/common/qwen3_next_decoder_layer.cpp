...
Qwen3NextDecoderImpl::Qwen3NextDecoderImpl(const ModelContext& context,
                                         int32_t layer_id) {
  ...
  if ((layer_id + 1) % 4 == 0) {
    attention_ = register_module(
      "self_attn",
      Qwen3NextAttention(model_args, quant_args, parallel_args, options));
  } else {
...
void Qwen3NextDecoderImpl::load_state_dict(const StateDict& state_dict) {
  if (attention_) {
...
torch::Tensor Qwen3NextDecoderImpl::forward(
    torch::Tensor& x,
  ...
  if (attention_) {
    ...
    x = linear_attention_->forward(x, attn_metadata, kv_cache);
  }
